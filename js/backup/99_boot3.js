  ...
  const DEFAULT_MIN_CARD_ZOOM = 0.1;
  const DEFAULT_MAX_CARD_ZOOM = 3.0;

  // ---------------------------
  // Portal wrappers (resizable containers)
  // ---------------------------
  const DEFAULT_PORTAL_W = 820;
  const DEFAULT_PORTAL_H = 820;

  function portalLabelFromId(cardId){
    const id = String(cardId || "").trim();
    return id ? id : "(unassigned)";
  }

  function createPortalShell(cardId){
    const portal = document.createElement("div");
    portal.className = "portal card-portal";
    portal.tabIndex = 0;

    // Default size can be overridden by CSS or user resize (inline wins after first drag).
    portal.style.width = portal.style.width || `${DEFAULT_PORTAL_W}px`;
    portal.style.height = portal.style.height || `${DEFAULT_PORTAL_H}px`;

    const tag = document.createElement("div");
    tag.className = "portal-tag";
    tag.setAttribute("data-role", "portal-tag");
    tag.textContent = portalLabelFromId(cardId);

    const stage = document.createElement("div");
    stage.className = "portal-stage";

    portal.appendChild(tag);
    portal.appendChild(stage);
    return portal;
  }

  function wrapCardShellInPortal(cardShell, cardId){
    const portal = createPortalShell(cardId || cardShell?.dataset?.cardId);
    portal.dataset.cardId = String(cardId || cardShell?.dataset?.cardId || "").trim();
    const stage = portal.querySelector(".portal-stage");
    if (stage && cardShell) stage.appendChild(cardShell);
    return portal;
  }
  ...
  function ensurePortals(){
    ...
    const shells = Array.from(container.querySelectorAll(".card-shell"));
    shells.forEach((shell) => {
      if (!shell || shell.closest(".portal")) return;
      const parent = shell.parentNode;
      const next = shell.nextSibling;
      const portal = wrapCardShellInPortal(shell, shell.dataset.cardId);
      if (next) parent.insertBefore(portal, next);
      else parent.appendChild(portal);
    });

    // Simple selection highlight.
    if (!container.__portalSelectionBound){
      Object.defineProperty(container, "__portalSelectionBound", {value:true, configurable:true});
      container.addEventListener("pointerdown", (ev) => {
        const portal = ev.target && ev.target.closest ? ev.target.closest(".portal") : null;
        if (!portal || !container.contains(portal)) return;
        container.querySelectorAll(".portal.is-selected").forEach((p) => p.classList.remove("is-selected"));
        portal.classList.add("is-selected");
      });
    }
  }
  ...
  function appendCardFragmentAsPortal(container, fragment, cardId){
    if (!container || !fragment) return {portal:null, root:null};
    const root = fragment.querySelector ? fragment.querySelector(".card-shell") : null;
    const portal = createPortalShell(cardId || root?.dataset?.cardId);
    portal.dataset.cardId = String(cardId || root?.dataset?.cardId || "").trim();
    const tag = portal.querySelector(".portal-tag");
    if (tag) tag.textContent = portalLabelFromId(portal.dataset.cardId);
    const stage = portal.querySelector(".portal-stage");
    if (stage) stage.appendChild(fragment);
    container.appendChild(portal);
    if (!container.querySelector(".portal.is-selected")){
      portal.classList.add("is-selected");
    }
    return {portal, root};
  }

  function readCssNumber(el, varName){
    if (!el) return NaN;
  ...
  function readZoomMode(card){
    const root = document.documentElement;
    const fromCard = getComputedStyle(card).getPropertyValue("--card-zoom-mode");
    const fromRoot = getComputedStyle(root).getPropertyValue("--card-zoom-mode");
    const mode = (String(fromCard || "").trim() || String(fromRoot || "").trim()).toLowerCase();
    return mode || "auto";
  }

  function readManualZoom(card){
    const root = document.documentElement;
    // 1) explicit dataset override
    const ds = card && card.dataset ? (card.dataset.zoom || card.dataset.cardZoom || "") : "";
    const n1 = parseFloat(String(ds).trim());
    if (Number.isFinite(n1) && n1 > 0) return n1;

    // 2) CSS var override on card
    const n2 = readCssNumber(card, "--card-zoom-manual");
    if (Number.isFinite(n2) && n2 > 0) return n2;

    // 3) CSS var override on root
    const n3 = readCssNumber(root, "--card-zoom-manual");
    if (Number.isFinite(n3) && n3 > 0) return n3;

    return NaN;
  }

  function readZoomClamp(card){
    const root = document.documentElement;
    const min = readCssNumber(card, "--card-zoom-min") || readCssNumber(root, "--card-zoom-min") || DEFAULT_MIN_CARD_ZOOM;
    const max = readCssNumber(card, "--card-zoom-max") || readCssNumber(root, "--card-zoom-max") || DEFAULT_MAX_CARD_ZOOM;
    const safeMin = Number.isFinite(min) ? min : DEFAULT_MIN_CARD_ZOOM;
    const safeMax = Number.isFinite(max) ? max : DEFAULT_MAX_CARD_ZOOM;
  ...
  function resolveCardContainer(card){
    if (!card) return document.documentElement;
    const stage = card.closest ? card.closest(".portal-stage") : null;
    if (stage) return stage;
    const portal = card.closest ? card.closest(".portal") : null;
    if (portal) return portal;
    return (card.closest ? card.closest(".card-container") : null) || document.documentElement;
  }
  ...
  function updateCardLayout(card){
    ...
    const baseW = container.clientWidth || rect.width;
    const baseH = container.clientHeight || rect.height;
    const viewportWidth = (Math.max(0, baseW - padX)) || idealWidth;
    const viewportHeight = (Math.max(0, baseH - padY)) || idealHeight;
    ...
    const zoomMode = readZoomMode(card);
    const clamp = readZoomClamp(card);

    // Manual mode: user (or UI) provides the zoom; we do NOT overwrite it with container-fitting logic.
    const manualZoom = readManualZoom(card);
    const unclampedZoom = (zoomMode === "manual" && Number.isFinite(manualZoom) && manualZoom > 0)
      ? manualZoom
      : safeViewportScale;

    // Clamp only in auto mode (manual is allowed to go anywhere positive unless you set --card-zoom-min/max)
    const clampedZoom = (zoomMode === "manual")
      ? unclampedZoom
      : Math.min(Math.max(unclampedZoom, clamp.min), clamp.max);
  ...
  function updateCardIdentity(root, cardId){
    ...
    const portal = root.closest && root.closest(".portal");
    if (portal){
      portal.dataset.cardId = safeId;
      const tag = portal.querySelector(".portal-tag");
      if (tag) tag.textContent = portalLabelFromId(safeId);
    }
...
function cloneCardTemplate(){
  ...
  if (container){
    while (getCardRoots().length < desiredCount){
      ...
      appendCardFragmentAsPortal(container, fragment, root && root.dataset ? root.dataset.cardId : "");
    }
    ...
    if (storedIds.length){
      ...
      for (let i = roots.length - 1; i >= desiredCount; i -= 1){
        const target = (roots[i] && roots[i].closest) ? (roots[i].closest(".portal") || roots[i]) : roots[i];
